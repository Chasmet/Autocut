...
const SEGMENT_KEY = "cutflow-segment-seconds-v4";
const DESIGN_KEY = "cutflow-custom-design-v4";
...
function darkerHex(hex, amount = 18) {
  const clean = String(hex || "#07111f").replace("#", "");
...
function getSavedDesign() {
  try {
    return { ...DEFAULT_DESIGN, ...JSON.parse(localStorage.getItem(DESIGN_KEY) || "{}") };
  } catch {
...
function applyDesign(design) {
  const safeDesign = { ...DEFAULT_DESIGN, ...design };
  setCssVar("--accent", safeDesign.accent);
  setCssVar("--accent-2", safeDesign.accent2);
  setCssVar("--button-main", safeDesign.button);
  setCssVar("--bg", safeDesign.bg);
  setCssVar("--bg-2", darkerHex(safeDesign.bg, 14));

  if (dom.accentColorInput) dom.accentColorInput.value = safeDesign.accent;
  if (dom.accent2ColorInput) dom.accent2ColorInput.value = safeDesign.accent2;
  if (dom.buttonColorInput) dom.buttonColorInput.value = safeDesign.button;
  if (dom.bgColorInput) dom.bgColorInput.value = safeDesign.bg;
}
...
function setupDesignControls() {
  ...
  [dom.accentColorInput, dom.accent2ColorInput, dom.buttonColorInput, dom.bgColorInput]
    .filter(Boolean)
    .forEach((input) => {
      input.addEventListener("input", () => {
        const design = getDesignFromInputs();
        applyDesign(design);
        saveDesign(design);
        setStatus("Couleurs enregistrées", null);
      });
      ...
      input.addEventListener("change", () => {
        ...
        applyDesign(design);
        saveDesign(design);
        setStatus("Couleurs conservées après rechargement", null);
      });
    });
...
function setupTheme() {
  ...
  dom.themeToggle.addEventListener("click", () => {
    const current = dom.body.getAttribute("data-theme") || "dark";
...
function setupSegmentControls() {
  ...
  const saved = Number(localStorage.getItem(SEGMENT_KEY) || 5);
  applySegmentSeconds(saved, "init");
  ...
  dom.applyCustomSegmentBtn?.addEventListener("click", () => {
    ...
    if (value > 300) {
      ...
      setStatus("Maximum réglé à 300 secondes", null);
      return;
    }
...
function setupParallax() {
  ...
  window.addEventListener("scroll", () => {
    ...
    window.requestAnimationFrame(() => {
      const offsetY = (window.scrollY || 0) * 0.12;
...
function setupFileInput() {
  dom.mediaFile.addEventListener("change", async (event) => {
    ...
    selectedFile = file;
    dom.fileInfo.innerHTML = `<div class="file-pill">${getMediaKind(file)} : ${file.name}</div>`;

    const url = URL.createObjectURL(file);
    dom.previewWrap.innerHTML = file.type.startsWith("video/")
...
function buildSegmentTimes(duration, segmentSize) {
  ...
  while (start < duration) {
    ...
    list.push({ index, start, end, length: end - start });
    start += segmentSize;
    index += 1;
  }
  return list;
...
function interleaveAudioBuffer(audioBuffer, startSample, endSample) {
  ...
  for (let i = 0; i < length; i += 1) {
    for (let channel = 0; channel < channels; channel += 1) {
      result[offset] = audioBuffer.getChannelData(channel)[startSample + i] || 0;
      offset += 1;
    }
  }
  return result;
...
async function saveCutsToDB(cuts) {
  ...
  return new Promise((resolve, reject) => {
    ...
    cuts.forEach((cut) => store.put(cut));
...
async function shareOrSaveCut(blob, fileName, mimeType, blobUrl) {
  ...
  try {
    if (navigator.canShare && navigator.canShare({ files: [file] }) && navigator.share) {
      await navigator.share({ files: [file], title: fileName, text: "CutFlow - fichier découpé" });
...
function createPlayerPreview(blobUrl, mimeType) {
  if (mimeType.startsWith("audio/")) return `<audio controls preload="metadata" src="${blobUrl}" style="width:100%; border-radius:14px;"></audio>`;
  if (mimeType.startsWith("video/")) return `<video controls playsinline preload="metadata" src="${blobUrl}" style="width:100%; border-radius:14px;"></video>`;
  return "";
}
...
function createResultCard({ index, start, end, blob, fileName, extension, mimeType }) {
  ...
  wrapper.innerHTML = `
    <div class="result-top"><div><h4 class="result-title">Cut ${index}</h4><div class="result-time">${formatSeconds(start)} à ${formatSeconds(end)}</div></div></div>
    <div class="result-badges"><span class="badge">${Math.round(end - start)}s</span><span class="badge">${extension.toUpperCase()}</span></div>
    ${createPlayerPreview(blobUrl, mimeType)}
    <div style="display:grid; grid-template-columns:1fr 1fr; gap:10px;"><button class="download-btn open-btn" type="button">Ouvrir</button><button class="download-btn share-btn" type="button">Partager / enregistrer</button></div>
    <button class="download-btn save-btn" type="button" style="background:linear-gradient(135deg,var(--accent),var(--accent-2));">Télécharger classique</button>
  `;
...
async function persistCurrentCuts() {
  ...
  try {
    ...
    saveSessionMeta({ savedAt: new Date().toISOString(), count: currentSessionCuts.length });
...
function renderSavedActionsIfNeeded() {
  ...
  box.id = "savedCutsBox";
  box.innerHTML = `<h4 class="result-title">Cuts sauvegardés</h4><div class="result-time">${meta.count} cut(s) enregistrés localement</div><div style="display:grid; grid-template-columns:1fr 1fr; gap:10px; margin-top:10px;"><button class="download-btn" id="reloadSavedCutsBtn" type="button">Recharger</button><button class="download-btn" id="deleteSavedCutsBtn" type="button">Tout effacer</button></div>`;
  ...
  box.querySelector("#deleteSavedCutsBtn").addEventListener("click", async () => {
    if (!window.confirm("Supprimer tous les cuts sauvegardés ?")) return;
    await clearSavedCutsFromDB();
...
async function restoreSavedCuts() {
  try {
    ...
    currentSessionCuts = cuts.sort((a, b) => a.index - b.index);
    currentSessionCuts.forEach((cut) => dom.resultsList.appendChild(createResultCard(cut).element));
    renderSavedActionsIfNeeded();
...
async function cutAudioNative(file) {
  ...
  const segments = buildSegmentTimes(audioBuffer.duration, selectedSegmentSeconds);
  dom.resultsList.innerHTML = "";
  currentSessionCuts = [];

  for (let i = 0; i < segments.length; i += 1) {
    const segment = segments[i];
    setStatus(`Découpe audio ${segment.index} sur ${segments.length}...`, ((i + 0.2) / segments.length) * 100);
    const blob = encodeWavFromAudioBuffer(audioBuffer, segment.start, segment.end);
    const fileName = `cut_${segment.index}.wav`;
    const { element, cutData } = createResultCard({ index: segment.index, start: segment.start, end: segment.end, blob, fileName, extension: "wav", mimeType: "audio/wav" });
    currentSessionCuts.push(cutData);
    dom.resultsList.appendChild(element);
    setStatus(`Cut audio ${segment.index} prêt`, ((i + 1) / segments.length) * 100);
  }
...
async function cutVideoWithFFmpeg(file) {
  ...
  for (let i = 0; i < segments.length; i += 1) {
    ...
    setStatus(`Découpe vidéo ${segment.index} sur ${segments.length}...`, ((i + 0.2) / segments.length) * 100);
    const args = outputExt === "webm"
      ? ["-ss", String(segment.start), "-t", String(segment.length), "-i", inputName, "-c:v", "libvpx", "-deadline", "realtime", "-cpu-used", "8", "-c:a", "libvorbis", outputName]
      : ["-ss", String(segment.start), "-t", String(segment.length), "-i", inputName, "-c:v", "libx264", "-preset", "ultrafast", "-c:a", "aac", "-movflags", "+faststart", outputName];
    await ffmpeg.exec(args);
    const data = await ffmpeg.readFile(outputName);
    const mimeType = outputExt === "webm" ? "video/webm" : "video/mp4";
    const blob = new Blob([data.buffer], { type: mimeType });
    const { element, cutData } = createResultCard({ index: segment.index, start: segment.start, end: segment.end, blob, fileName: outputName, extension: outputExt, mimeType });
    currentSessionCuts.push(cutData);
    dom.resultsList.appendChild(element);
    try { await ffmpeg.deleteFile(outputName); } catch {}
    setStatus(`Cut vidéo ${segment.index} prêt`, ((i + 1) / segments.length) * 100);
  }
...
async function startCutting() {
  ...
  clearResults();
  setStatus(`Préparation découpe ${selectedSegmentSeconds}s...`, 2);
  try {
    if (selectedFile.type.startsWith("audio/")) await cutAudioNative(selectedFile);
    else if (selectedFile.type.startsWith("video/")) await cutVideoWithFFmpeg(selectedFile);
